...
import com.learn.summer.web.controller.ApiController;
import com.learn.summer.web.controller.ControllerConfiguration;
...
public class DispatcherServletTest {
    ...
    void getHello() throws ServletException, IOException {
        var req = createMockRequest("GET", "/hello/alice", null, null);
        var resp = createMockResponse();
        this.dispatcherServlet.service(req, resp);
        assertEquals(200, resp.getStatus());
        assertEquals("Hello, alice", resp.getContentAsString());
    }

    @Test
    void getApiHello() throws ServletException, IOException {
        var req = createMockRequest("GET", "/api/hello/alice", null, null);
        var resp = createMockResponse();
        this.dispatcherServlet.service(req, resp);
        assertEquals(200, resp.getStatus());
        assertEquals("{\"name\":\"alice\"}", resp.getContentAsString());
    }

    @Test
    void getGreeting() throws ServletException, IOException {
        var req = createMockRequest("GET", "/greeting", null, Map.of("name", "alice"));
        var resp = createMockResponse();
        this.dispatcherServlet.service(req, resp);
        assertEquals(200, resp.getStatus());
        assertEquals("Hello, alice", resp.getContentAsString());
    ...
    void getApiGreeting() throws ServletException, IOException {
        var req = createMockRequest("GET", "/api/greeting", null, Map.of("name", "alice"));
        var resp = createMockResponse();
        this.dispatcherServlet.service(req, resp);
        assertEquals(200, resp.getStatus());
        assertEquals("application/json", resp.getContentType());
        assertEquals("{\"action\":{\"name\":\"alice\"}}", resp.getContentAsString());
    }
    ...
    void init() throws ServletException {
        ...
        var apiFilter = applicationContext.getBean(ApiFilterRegistrationBean.class);
    ...
    MockServletContext createMockServletContext() {
        ...
        ctx.setRequestCharacterEncoding(StandardCharsets.UTF_8);
        ctx.setResponseCharacterEncoding(StandardCharsets.UTF_8);
        return ctx;
    }

    MockHttpServletRequest createMockRequest(String method, String path, Object body, Map<String, String> param) {
        var req = new MockHttpServletRequest(this.ctx, method, path);
        if(method.equals("GET") && param != null) {
            param.keySet().forEach(key ->
                    req.addParameter(key, param.get(key)));
